"""
Task Agent - GROQ Implementation
Uses GROQ API with Llama models for better rate limits
"""
...
async def run_agent(user_message: str, user_id: str) -> str:
    """
    Run the agent with a user message using GROQ API.
    """
    try:
        # Get GROQ API key
...
1. list_my_tasks_args - List all tasks (call with no parameters)
2. add - Create a new task (requires name, optional priority)
3. remove - Delete a task (requires exact or partial name match)
4. update - Update task status or priority (requires exact or partial name match)

IMPORTANT INSTRUCTIONS:
- When user says "list", "show", or "display" tasks, call list_my_tasks_args with NO parameters
- When user wants to delete/remove a task, extract the task name carefully from their message
- When user wants to mark a task done, call update with status="completed"
- When user wants to change priority, call update with the new priority
...
Always show tool results to the user. Be friendly and conversational."""
        ...
        # Call the API with GROQ's lighter Llama model
        ...
        # Get final response with tool results using lighter model
